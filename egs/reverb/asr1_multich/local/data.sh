...
enable_pesq=false # please make sure that you or your institution have the proper license to use PESQ before turning on this flag
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for setname in tr_simu_1ch tr_simu_2ch tr_simu_8ch; do
        sed -i -e "s#${datadir}/data/mc_train/#${datadir}/data/mc_train/observation/#g" data/${setname}/wav.scp
    done
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for setname in $tasks; do
        if $use_reverb_reference; then
            sed -e "s#${datadir}/data/mc_train/observation/#${datadir}/data/mc_train/reverb_source/#g" \
                data/${setname}/wav.scp > data/${setname}/spk1.scp
        else
            # single-channel source signal
            sed -e "s#${datadir}/data/mc_train/observation/#${REVERB_OUT}/WSJCAM0/data/#g" \
                -e "s#sox -M ##g" \
                -e 's# [^[:space:]]*_ch2.*$##g' \
                -e "s#_ch[0-9]\.wav\>#.wav#g" \
                data/${setname}/wav.scp > data/${setname}/spk1.scp
        fi
        sed -e "s#${datadir}/data/mc_train/observation/#${datadir}/data/mc_train/early/#g" \
            data/${setname}/wav.scp > data/${setname}/dereverb1.scp
        sed -e "s#${datadir}/data/mc_train/observation/#${datadir}/data/mc_train/noise/#g" \
            data/${setname}/wav.scp > data/${setname}/noise1.scp
    done
    for subset in dt et; do
        tasks="${subset}_simu_1ch ${subset}_simu_2ch ${subset}_simu_8ch ${subset}_simu_${nch_se}ch_multich"
        datadir="${REVERB}/REVERB_WSJCAM0_${subset}"
        for setname in $tasks; do
            if $use_reverb_reference; then
                sed -e "s#${datadir}/data/mc_train/observation/#${datadir}/data/mc_train/reverb_source/#g" \
                    data/${setname}/wav.scp > data/${setname}/spk1.scp
                ...
                # single-channel source signal
                sed -e "s#${datadir}/data/far_test/#${datadir}/data/cln_test/#g" \
                    -e "s#sox -M ##g" \
                    -e 's# [^[:space:]]*_ch2.*$##g' \
                    -e "s#_ch[0-9]\.wav\>#.wav#g" \
                    data/${setname}/wav.scp > data/${setname}/spk1.scp
