...
def main(args):
    ...
    for utt, wavs in dataset.items():
        ...
        metrics_dict1 = {"srmr": np.array([srmr(w, sr) for w in wav_sep1])}
        ...
        metrics_dict = {
            "input_srmr": np.array([srmr(wav_mix, sr)]),
            "srmr": np.array([srmr(w, sr) for w in wav_enh]),
        }
    ...
    for k, v in eval_results1.items():
        logging.warning('mean {}: {}'.format(k.replace('_', '-').upper(), float(np.mean(v))))
    print("\n", flush=True)
