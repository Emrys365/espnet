...
def get_parser(parser=None, required=True):
    if parser is None:
        parser = configargparse.ArgumentParser(
            description="Train a frontend model on one CPU, one or multiple GPUs",
    ...
    parser.add_argument("--load-input-lengths", type=strtobool, default=False,
    ...
    parser.add_argument('--normalization', type=strtobool, default=False,
                        help='Normalize the ouput speech and reference to max=1')
    ...
    parser.add_argument('--mask-loss', type=str, default='mse', choices=['mse', 'l1', 'smooth_l1'],
                        help='loss function when using masks as the target')
    parser.add_argument('--mask-type', type=str, default='PSM^2',
                        choices=["IBM", "IRM", "cIRM", "IAM", "IAM^2", "VAD", "VAD^2", "PSM", "NPSM", "PSM^2"],
                        help='mask type')
    parser.add_argument('--enh-loss-type', type=str, default='mask_mse',
                        choices=["mask_mse", "spectrum", "snr", "si_snr", "ci_sdr"],
                        help='enh loss type')
...
#            filename='{}/details.log'.format(args.debugdir), filemode='w',
        logging.basicConfig(
    ...
    if args.num_spkrs == 1:
        ...
        if args.backend == "pytorch":
            ...
            if args.target_is_mask and args.train_mask_only:
                logging.warning('(only train MaskEstimator)')
            from ss_train import train
            train(args)
        else:
