...
train_set=tr_spatialized_reverb_multich_16k_max
train_aux_set=train_si284
train_dev=cv_spatialized_reverb_multich_16k_max
train_test=tt_spatialized_reverb_multich_16k_max
recog_set="tt_spatialized_reverb_multich_16k_max"
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    (
        ...
        if [[ $ngpu -eq 0 ]]; then
            ...
            if [ "$rtask" = "${train_dev}" ]; then
            #for jobid in 2 8 9 10 20; do
            #(

            #${decode_cmd} JOB=$jobid ${expdir}/${decode_dir}/log/decode.JOB.log \
            ...
            CUDA_VISIBLE_DEVICES=0 ${decode_cmd} --gpu ${ngpu} --qos qd3 ${expdir}/${decode_dir}/log/decode.log \
                asr_recog.py \
                --num-spkrs ${num_spkrs} \
                --config ${decode_config} \
                --ngpu ${ngpu} \
