...
data_type = {'tr','cv','tt'};
wsjroot = '/db/processed/public/WSJWAV_full/'; % YOUR_PATH/, the folder containing wsj0/ and wsj1/
output_dir16k='/xxx/wsj1-mix/2speakers/wav16k';
output_dir8k='/xxx/wsj1-mix/2speakers/wav8k';
...
for i_mm = 1:length(min_max)
    for i_type = 1:length(data_type)
        ...
        for i = 1:num_files
            ...
            % get input wavs
            if useaudioread
                [s1, fs] = audioread([wsjroot C{1}{i}]);
                s2       = audioread([wsjroot C{3}{i}]);
            else                
                [s1, fs] = wavread([wsjroot C{1}{i}]); %#ok<*DWVRD>
                s2       = wavread([wsjroot C{3}{i}]);            
            end
