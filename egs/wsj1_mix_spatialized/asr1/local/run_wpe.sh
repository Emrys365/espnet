...
array=$3
task=`basename $sdir`
expdir=exp/wpe_1iter/${task}_${array}chs
...
#cat $expdir/channels_input | \
#
# convert the single wav path in each line in wav.list
#    into two wav paths (CH1 & CH2) in each line
cat ${odir}/../wav.list | \
awk -v dir=${odir} -v array=array '{n=split($1, lst, "/"); gsub(/.wav/, "", lst[n]); for (i=1; i<=2; i++) {printf("%s/%s.CH%d.wav ", dir, lst[n], i)} print("");}' > $expdir/channels_output
#paste -d" " $expdir/channels_input $expdir/channels_output > $output_wavfiles
paste -d" " $odir/../wav.list $expdir/channels_output > $output_wavfiles
