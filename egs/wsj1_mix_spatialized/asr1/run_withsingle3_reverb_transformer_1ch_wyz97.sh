...
#train_config=conf/tuning/train_multispkr_trans_wyz97_padertorch.yaml
#train_config=conf/tuning/train_multispkr_trans_wyz97_padertorch_mvdr_nowpe.yaml
#train_config=conf/tuning/train_multispkr_trans_wyz97_padertorch_mvdr.yaml
#train_config=conf/tuning/train_multispkr_trans_wyz97_padertorch_mvdr_atf.yaml
#train_config=conf/tuning/train_multispkr_trans_wyz97_padertorch_wmpdr_atf.yaml
#train_config=conf/tuning/train_mpdr_wpe.yaml
#train_config=conf/tuning/train_mvdr_wpe_trans.yaml
#train_config=conf/tuning/train_mvdr_trans.yaml
...
train_set=tr_spatialized_reverb_multich #_naraWPE3 #tr_spatialized_reverb_multich #train_si284
train_dev=cv_spatialized_reverb_multich #/nara_WPE_dereverb #_1iter #test_dev93
train_test=tt_spatialized_reverb_multich #/nara_WPE_dereverb #_1iter #test_eval92
recog_set="tt_spatialized_reverb_multich" #/nara_WPE_dereverb" #_1iter"
...
if [ -z ${tag} ]; then
    expname=${train_set}2c_${backend}_$(basename ${train_config%.*})_$(basename ${preprocess_config%.*})${init_frontend:+_init_frontend}${init_asr:+_init_asr}${lr:+lr$lr}
...
#expdir=exp_revb/seed${seed}_${expname}${multich_epochs:+_fromE$multich_epochs}_5taps_$(date +"%Y_%m_%d")
if [ -n "$test_nmics" ]; then
    chs="${test_nmics}ch_"
else
    chs=
fi
#expdir=exp/seed${seed}_${expname}${multich_epochs:+_fromE$multich_epochs}_${chs}5taps${use_vad_mask:+_vad_mask}${bf_wpe_tag:+_tag_$bf_wpe_tag}_$(date +"%Y_%m_%d")
expdir=exp/seed${seed}_${expname}${multich_epochs:+_fromE$multich_epochs}_${chs}5taps${use_vad_mask:+_vad_mask}${bf_wpe_tag:+_tag_$bf_wpe_tag}_$(date +"%Y_%m_%d")
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    if [ -f "${expdir}/train.log" ]; then
        count=1
        while [ -f "${expdir}/train.${count}.log" ]; do
            let count=count+1
        done
        mv "${expdir}/train.log" "${expdir}/train.${count}.log"
    fi
    if [ -f "${expdir}/results/log" ]; then
        count=1
        while [ -f "${expdir}/results/log.bak${count}" ]; do
            let count=count+1
        done
        mv "${expdir}/results/log" "${expdir}/results/log.bak${count}"
    fi
    for fname in acc loss loss_main loss_ctc loss_att; do
        if [ -f "${expdir}/results/${fname}.png" ]; then
            count=1
            while [ -f "${expdir}/results/${fname}.bak${count}.png" ]; do
                let count=count+1
            done
            mv "${expdir}/results/${fname}.png" "${expdir}/results/${fname}.bak${count}.png"
        fi
    done
	...
	${cuda_cmd} --qos qd3 --gpu ${ngpu} ${expdir}/train.log \
        asr_train.py \
        --config ${train_config} \
        --ngpu ${ngpu} \
        --backend ${backend} \
...
#expdir=exp_revb/seed1_tr_spatialized_reverb_multich_naraWPE3_singlespkr2c_pytorch_train_mvdr_wpe_trans_preprocess_init_asr_globalcmvn_5taps_2020_05_09
#expdir=exp_revb/seed1_tr_spatialized_reverb_multich_naraWPE3_singlespkr2c_pytorch_train_mvdr_wpe_trans_preprocess_globalcmvn_fromE5_5taps_2020_05_07
#recog_model=snapshot.ep.27
lmexpdir=exp/train_rnnlm_pytorch_lm_word65000

#expdir=exp_revb_icassp2020/seed1_tr_spatialized_reverb_multich_singlespkr2c_pytorch_train_multispkr_trans_wyz97_padertorch_preprocess_init_frontend_init_asr_globalcmvn_5taps_2020_09_08
#expdir=exp_revb_icassp2020/seed1_tr_spatialized_reverb_multich_singlespkr2c_pytorch_train_multispkr_trans_wyz97_padertorch_preprocess_init_frontend_init_asr_globalcmvn_5taps_2020_09_09
#recog_model=snapshot.ep.24

#expdir=exp_revb_icassp2020/seed1_tr_spatialized_reverb_multich_singlespkr2c_pytorch_train_multispkr_trans_wyz97_padertorch_mvdr_preprocess_init_frontend_init_asr_globalcmvn_5taps_2020_09_08
#recog_model=snapshot.ep.19

#expdir=exp_revb_icassp2020/seed1_tr_spatialized_reverb_multich_singlespkr2c_pytorch_train_multispkr_trans_wyz97_padertorch_mvdr_atf_preprocess_globalcmvn_5taps_2020_09_15
...
#        decode_dir=decode${prefix}_${rtask}_$(basename ${decode_config%.*})_${recog_model}_${lmtag}
        ...
        #decode_dir=decode_wpd5_${rtask}_$(basename ${decode_config%.*})_${lmtag}
