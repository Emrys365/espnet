...
#train_config=conf/tuning/train_multispkr512_trans_wyz97.yaml
...
recog_set="tt_spatialized_reverb_multich" #/nara_WPE_dereverb"
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    mkdir -p data/tr_spatialized_reverb_multich_naraWPE_singlespkr
#    concatjson.py data/tr_spatialized_reverb_multich/nara_WPE_dereverb/data.json data/train_si284/data.json data/tr_spatialized_reverb_multich/data.json > data/tr_spatialized_reverb_multich_naraWPE2_singlespkr/data.json
...
#train_set=tr_spatialized_reverb_multich_naraWPE2_singlespkr
...
if [ -z ${tag} ]; then
    if [ -n "$init_from_mdl" ]; then
        expname=seed${seed}_${train_set}2c_${backend}_$(basename ${train_config%.*})_$(basename ${preprocess_config%.*})_init_from_mdl${lr:+_lr$lr}_wpd_ver${wpd_opt}
    else
        expname=seed${seed}_${train_set}2c_${backend}_$(basename ${train_config%.*})_$(basename ${preprocess_config%.*})${init_frontend:+_init_frontend}${init_asr:+_init_asr}${lr:+_lr$lr}_wpd_ver${wpd_opt}
...
expdir=exp_revb_icassp2020/${expname}${multich_epochs:+_fromE$multich_epochs}_${chs}5taps${bf_wpe_tag:+_tag_$bf_wpe_tag}_$(date +"%Y_%m_%d")
#expdir=exp_revb/seed1_tr_spatialized_reverb_multich_singlespkr2c_pytorch_train_multispkr512_trans_wyz97_preprocess_wpd_ver5_globalcmvn_2020_04_27
#expdir=exp_revb/seed1_tr_spatialized_reverb_multich_singlespkr2c_pytorch_train_multispkr512_trans_wyz97_preprocess_wpd_ver5_globalcmvn_fromE5_5taps_2020_05_03
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
	...
	${cuda_cmd} --qos qd7 --gpu ${ngpu} ${expdir}/train.log \
        asr_train.py \
        --config ${train_config} \
        --ngpu ${ngpu} \
        --backend ${backend} \
...
#expdir=exp_revb/tr_spatialized_reverb_multich_singlespkr_pytorch_train_multispkr_wyz97_preprocess_init_frontend_wpd_ver5_2020_03_25
#expdir=exp_revb/tr_spatialized_reverb_multich_singlespkr_pytorch_train_multispkr_wyz97_preprocess_wpd_ver5_2020_03_25
#expdir=exp_revb/tr_spatialized_reverb_multich_singlespkr_pytorch_train_multispkr_wyz97_preprocess_init_frontend_init_asr_wpd_ver5_2020_03_25

#expdir=exp_revb/seed1_tr_spatialized_reverb_multich_singlespkr2c_pytorch_train_multispkr512_trans_wyz97_preprocess_wpd_ver5_globalcmvn_5taps_2020_05_12
#ctc_weight=0.2
#lm_weight=1.0
...
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/filter_samples.log \
        ...
        --verbose ${verbose} \
        --resume ${resume} \
        --train-json data/${train_set}/data_2c.json \
