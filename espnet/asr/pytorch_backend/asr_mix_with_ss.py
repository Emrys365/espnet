...
class CustomConverter(object):
    ...
    def __init__(self, subsampling_factor=1, dtype=torch.float32):
        ...
        print("Converter: subsampling_factor={}".format(subsampling_factor), flush=True)
        self.ignore_id = -1
        self.dtype = dtype
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        if len(batch[0]) == 2:
            # xs: (B, T, F)
            xs, ys = batch[0]
            tgt_s1, tgt_s2, wav_lens = None, None, None
#            logging.warning('single-channel input')
#            logging.warning(f'xs: {[x.shape for x in xs]}')
        elif len(batch[0]) == 3:
            ...
            # xs / tgt_s1 / tgt_s2: (B, T, C, F)
            xs, tgt_s1, tgt_s2, ys = batch[0]
            wav_lens = None
#            logging.warning('multi-channel input')
        else:
            xs, tgt_s1, tgt_s2, ys, wav_lens = batch[0]
        ...
        if self.subsampling_factor > 1:
            ...
            tgt_s1 = [tgt[::self.subsampling_factor] for tgt in tgt_s1] if tgt_s1 is not None else None
            tgt_s2 = [tgt[::self.subsampling_factor] for tgt in tgt_s2] if tgt_s2 is not None else None

#        if len(batch[0]) != 2:
#            logging.warning('xs: {}, tgt: {}'.format([x.shape for x in xs], {"tgt1": [tgt.shape for tgt in tgt_s1], "tgt2": [tgt.shape for tgt in tgt_s2]}))
        ...
        if tgt_s1 is not None:
            if tgt_s1[0].dtype.kind == 'c':
                # (B, Tmax, F)
                tgt_pad_s1_real = pad_list(
                    [torch.from_numpy(y.real).float() for y in tgt_s1], 0).to(device, dtype=self.dtype)
                tgt_pad_s2_real = pad_list(
                    [torch.from_numpy(y.real).float() for y in tgt_s2], 0).to(device, dtype=self.dtype)
                tgt_pad_s1_imag = pad_list(
                    [torch.from_numpy(y.imag).float() for y in tgt_s1], 0).to(device, dtype=self.dtype)
                tgt_pad_s2_imag = pad_list(
                    [torch.from_numpy(y.imag).float() for y in tgt_s2], 0).to(device, dtype=self.dtype)
                # (num_spkrs, B, Tmax, F)
                tgt_pad_real = torch.stack((tgt_pad_s1_real, tgt_pad_s2_real), dim=0)
                tgt_pad_imag = torch.stack((tgt_pad_s1_imag, tgt_pad_s2_imag), dim=0)
                tgt_pad = {'real': tgt_pad_real, 'imag': tgt_pad_imag}
            else:

                # (B, Tmax, F)
                tgt_pad_s1 = pad_list(
                    [torch.from_numpy(y).float() for y in tgt_s1], 0).to(device, dtype=self.dtype)
                tgt_pad_s2 = pad_list(
                    [torch.from_numpy(y).float() for y in tgt_s2], 0).to(device, dtype=self.dtype)
                # (num_spkrs, B, Tmax, F)
                tgt_pad = torch.stack((tgt_pad_s1, tgt_pad_s2), dim=0)
        else:
            tgt_pad = None
        ...
        return xs_pad, ilens, ys_pad, tgt_pad, wav_lens
...
def load_pretrained_modules(model_path, target_model, match_keys, freeze_parms=False):
    ...
    if freeze_parms:
        print('Freezing parameters in {}'.format(match_keys), flush=True)
...
#    converter = CustomConverter(subsampling_factor=subsampling_factor, dtype=dtype)
    ...
    load_tr = LoadInputsAndTargets(
        mode='asr', load_input_lengths=args.load_input_lengths, load_output=True,
...
#        num_workers=args.n_iter_processes,
    ...
    trainer.extend(extensions.PlotReport(['main/loss_enh', 'validation/main/loss_enh'],
                                         'epoch', file_name='loss_enh.png'))
    ...
    report_keys = ['epoch', 'iteration', 'main/loss', 'main/loss_ctc', 'main/loss_att', 'main/loss_enh',
                   'validation/main/loss', 'validation/main/loss_ctc', 'validation/main/loss_att', 'validation/main/loss_enh',
...
def recog(args):
    ...
    load_inputs_and_targets = LoadInputsAndTargets(
        mode='asr', load_input_lengths=train_args.load_input_lengths,
        load_wav_ref=train_args.load_wav_ref,
