...
from espnet.asr.asr_mix_utils import add_results_to_json as add_mix_results_to_json
from espnet.asr.asr_mix_utils import add_results_to_json_wer as add_mix_results_to_json_wer
...
def recog_mix_v2(args):
    ...
    if getattr(args, 'test_btaps', -1) > 0:
        if hasattr(model.frontend, 'btaps'):
            ...
            logging.warning('using btaps: {}'.format(args.test_btaps))
        elif hasattr(model.frontend, 'taps'):
            model.frontend.taps = args.test_btaps
            logging.warning('using taps: {}'.format(args.test_btaps))
        if hasattr(model.frontend, 'wpe'):
            model.frontend.wpe.taps = args.test_btaps
            logging.warning('using wpe taps: {}'.format(args.test_btaps))
    ...
    with torch.no_grad():
        for idx, name in enumerate(js.keys(), 1):
            ...
            batch = [(name, js[name])]
            feat = load_inputs_and_targets(batch)[0][0]
            enc_output = model.encode_raw(to_torch_tensor(feat).to(device=device))
            nbest_hyps = []
            for enc in enc_output:
                nbest_hs = beam_search(
                    x=enc.squeeze(0), maxlenratio=args.maxlenratio, minlenratio=args.minlenratio
                )
                nbest_hs = [
                    h.asdict() for h in nbest_hs[: min(len(nbest_hs), args.nbest)]
                ]
                nbest_hyps.append(nbest_hs)

            new_js[name] = add_mix_results_to_json(
                js[name], nbest_hyps, train_args.char_list
            )
