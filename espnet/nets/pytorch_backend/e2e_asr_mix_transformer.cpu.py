...
class E2E(E2E_ASR, ASRInterface, torch.nn.Module):
    ...
    def beamformer_add_arguments(parser):
        ...
        # rnn-based beamfomer
        group.add_argument('--beamformer-type', type=str, default="mvdr", choices=["mvdr", "mpdr"],
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        if self.frontend is not None:
            ...
            xs_pad = to_torch_tensor(xs_pad).cpu()
            #logging.warning(xs_pad.dtype)
            self.frontend.to(device='cpu')
            if self.tBPTT:
                with torch.no_grad():
                    hs_pad, hlens, mask = self.frontend(xs_pad, ilens.cpu())
                speech_truncated, olens, truncate_frames, frame_offsets = self.truncate(xs_pad, ilens)
                hs_pad_chunk, hlens_chunk, mask_chunk = self.frontend(speech_truncated, olens.cpu())
                ...
                hs_pad, hlens, mask = self.frontend(xs_pad, ilens.cpu())
            #hs_pad, hlens, mask = self.frontend(xs_pad, ilens.cpu())
            hlens = hlens.to(device)
            if isinstance(hs_pad, list):
                # move to GPU for ASR
                hs_pad = [hs.to(device) for hs in hs_pad]
...
#            hs, hlens = hs.float(), ilens
        ...
        if self.frontend is not None:
            ...
            #logging.warning(xs_pad.dtype)
            self.frontend.to(device='cpu')
            hs, hlens, mask = self.frontend(xs_pad, ilens)
            hlens = hlens.to(device)
            if isinstance(hs, list):
                # move to GPU for ASR
                hs = [h_.to(device) for h_ in hs]
                hlens_n = [None] * self.num_spkrs
                for i in range(self.num_spkrs):
                    hs[i], hlens_n[i] = self.feature_transform(hs[i].float(), hlens)
                hlens = hlens_n
    ...
    def enhance(self, feat):
        ...
        self.frontend.to(device='cpu')
        enhanceds, hlens, masks = self.frontend(xs_pad.cpu(), ilens)
        if prev:
            self.train()
    ...
    def calculate_all_attentions(self, xs_pad, ilens, ys_pad):
        ...
        with torch.no_grad():
            ...
            if self.frontend is not None:
                ...
                self.frontend.to(device='cpu')
                hs_pad, hlens, mask = self.frontend(xs_pad.cpu(), ilens.cpu())
                hlens = hlens.to(device)
                if isinstance(hs_pad, list):
                    hs_pad = [h_.to(device) for h_ in hs_pad]
                    hlens_n = [None] * self.num_spkrs
                    for i in range(self.num_spkrs):
                        hs_pad[i], hlens_n[i] = self.feature_transform(hs_pad[i].float(), hlens)
                    hlens = hlens_n
