...
#        logging.warning(f'src_mask:{src_mask.shape}, hs_pad:{hs_pad.shape}')
        hs_pad, hs_mask = self.encoder(hs_pad, src_mask)  # list: speaker differentiate
        self.hs_pad = hs_pad
        ...
        batch_size = hs_pad[0].size(0)
        ys_pad = ys_pad.transpose(0, 1)  # (num_spkrs, B, Lmax)
        hs_len = [hs_mask[i].view(batch_size, -1).sum(1) for i in range(self.num_spkrs)]
#        logging.warning(f'hs_pad[0]:{hs_pad[0].device}, hs_len[0]:{hs_len[0].device}, ys_pad[0]:{ys_pad[0].device}')
