...
def get_masked_covariance(Y: ComplexTensor,
    ...
    Psi = signal_framing(
        Y, btaps + 1, 1, bdelay, do_padding=True)[..., :T - bdelay - btaps + 1, :]
    ...
    mask_bar = mask[..., :T - bdelay - btaps + 1]

    # let T' = T - bdelay - btaps + 1
    # (B, F, C, T', btaps + 1) x (B, F, C, T', btaps + 1) -> (B, F, T', btaps + 1, C, btaps + 1, C)
    covariance_matrix = FC.einsum('bfdtk,bfetl->bftkdle', (Psi_conj_norm, Psi))
...
def get_covariances(Y: ComplexTensor,
    ...
    covariance_matrix = FC.einsum('bfdtk,bfetl->bfkdle', (Psi_conj_norm, Psi))
    ...
    if get_vector:
        ...
        covariance_vector = FC.einsum(
            'bfdtk,bfet->bfked', (Psi_conj_norm, Y[..., bdelay + btaps - 1:]))
