...
def get_weighted_power_spectral_density_matrix(
    ...
    Rd = np.matmul(X_inverse_power, hermite(X))
...
#     atf = FC.matmul(psd_noise, eigenvec.unsqueeze(-1))
    try:
        psd_noise_i = (psd_noise + epsilon * eye + eps).inverse()
    except:
        eps2 = 1e-4
        try:
            psd_noise = psd_noise / 10e+4
            psd_speech = psd_speech / 10e+4
            psd_noise += eps2 * eye
            psd_noise_i = psd_noise.inverse()
        except:
            try:
                psd_noise = psd_noise / 10e+10
                psd_speech = psd_speech / 10e+10
                psd_noise += eps2 * eye
                psd_noise_i = psd_noise.inverse()
            except:
                raise Exception('psd not invertable.')

    # phi: (..., C_1, C_2) x (..., C_2, C_3) -> (..., C_1, C_3)
    phi = FC.einsum("...ec,...cd->...ed", [psd_noise_i, psd_speech])
