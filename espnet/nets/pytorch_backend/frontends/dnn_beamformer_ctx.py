...
from espnet.nets.pytorch_backend.frontends.mask_estimator_ctx import MaskEstimator
...
class DNN_Beamformer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.mask = MaskEstimator(
            ...
            ctx_dim=ctx_dim, num_ctx=num_ctx
        )
    ...
    def forward(
        self, data: ComplexTensor, ilens: torch.LongTensor, ctxs: torch.FloatTensor = None
        ...
        Args:
            ...
            ctxs (torch.FloatTensor): (num_spkr, B, ctx_dim, C, T)
        Returns:
        ...
        # mask: (B, F, C, T)
        masks, _ = self.mask(data.float(), ilens, ctxs=ctxs)
        assert self.nmask == len(masks)
        masks = [m.double() for m in masks]
        masks = [m / m.abs().max(dim=-1, keepdim=True)[0].detach() for m in masks]
