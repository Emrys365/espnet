...
from espnet.nets.pytorch_backend.frontends.beamformer_diag_loading import apply_beamforming_vector
from espnet.nets.pytorch_backend.frontends.beamformer_diag_loading import get_mvdr_vector
from espnet.nets.pytorch_backend.frontends.beamformer_diag_loading import (
...
class DNN_Beamformer(torch.nn.Module):
    ...
    def forward(
        ...
        def apply_beamforming(data, ilens, psd_speech, psd_noise):
            ...
            ws = get_mvdr_vector(psd_speech, psd_noise, u.float())
        ...
        if targets is not None:
            masks = [m.float() for m in targets]
            assert self.nmask == len(masks)
        else:
            masks, _ = self.mask(data.float(), ilens)
            assert self.nmask == len(masks)
#            masks = [torch.clamp(m, min=1e-2).float() for m in masks]
...
class AttentionReference(torch.nn.Module):
    ...
    def forward(
        ...
        # Calculate amplitude
        psd_feat = (psd.real ** 2 + psd.imag ** 2 + 1e-8) ** 0.5
