...
from espnet.nets.pytorch_backend.frontends.dnn_beamformer_ctx import DNN_Beamformer
...
class Frontend(nn.Module):
    ...
    def forward(
        self, x: ComplexTensor, ilens: Union[torch.LongTensor, numpy.ndarray, List[int]], ctxs: torch.FloatTensor = None
...
#                    choices.append((True, False))
            ...
            if use_beamformer:
                if self.use_ctx and ctxs is not None:
                    ...
                    # h: (B, T, C, F) -> h: (B, T, F)
                    h, ilens, mask = self.beamformer(h, ilens, ctxs=ctxs)
                else:
