...
def _compress_mask(mask, K: float = 10, C: float = 0.1):
    ...
    """
    if isinstance(mask, ComplexTensor):
        return ComplexTensor(
            K * torch.tanh(mask.real * C / 2),
            K * torch.tanh(mask.imag * C / 2)
        )
    else:
...
def _revert_compressed_mask(mask, K: float = 10, C: float = 0.1):
    ...
    if is_torch_1_7_plus:
        ...
        atanh = lambda x: torch.log1p(2 * x / (1 - x)) * 0.5
    if isinstance(mask, ComplexTensor):
        return ComplexTensor(
...
class Frontend(nn.Module):
    def __init__(
        ...
        if not self.use_vad_mask:
            from espnet.nets.pytorch_backend.frontends.mask_estimator_v2_complex import ComplexMaskEstimator
            logging.warning('Using complex T-F masks')
        else:
            raise NotImplementedError("Complex VAD mask is not supported")
            # from espnet.nets.pytorch_backend.frontends.mask_estimator_vad_complex import ComplexMaskEstimator
            # logging.warning('Using complex VAD-like masks (same value for all frequencies in each frame)')

        self.mask = ComplexMaskEstimator(
        ...
        if self.use_beamformer:
            ...
            if beamformer_type not in (
                'mvdr', 'mpdr', 'wmpdr', 'mvdr_souden', 'mpdr_souden', 'wmpdr_souden', 'wpd_souden', 'wpd'
            ):
                raise ValueError(
        ...
        for i in range(self.iterations):
            ...
            if i == 0 and self.use_dnn_mask_for_wpe:
                ...
                if isinstance(irms, list):
                    mask = complex_clamp(irms[0], abs_min=1e-6).double()
                else:
                    mask = complex_clamp(irms, abs_min=1e-6).double()
            ...
            power = enhanced.real.pow(2) + enhanced.imag.pow(2)
...
#                u[..., self.ref_channel].fill_(1)
        ...
        # mask: (B, F, C, T)
        assert irms is not None
        masks = [complex_clamp(m.double(), abs_min=1e-2) for m in irms]

        if len(masks) == 2:  # (mask_speech, mask_noise)
            mask_speech, mask_noise = masks

            speech_signal = data * mask_speech
            noise_signal = data * mask_noise
            # (B, F, 1, 1)
            denom_speech = (speech_signal.real.pow(2) + speech_signal.imag.pow(2)).sum(
                dim=(-1, -2), keepdims=True
            )
            denom_noise = (noise_signal.real.pow(2) + noise_signal.imag.pow(2)).sum(
                dim=(-1, -2), keepdims=True
            )

            # covariance of source speech
            # psd_speech = get_power_spectral_density_matrix(data, mask_speech)
            psd_speech = FC.einsum('...ct,...et->...ce', [speech_signal, speech_signal.conj()]) / denom_speech
            if self.beamformer_type in ('mvdr', 'mvdr_souden'):
                # covariance of noise
                # psd_noise = get_power_spectral_density_matrix(data, mask_noise)
                psd_noise = FC.einsum('...ct,...et->...ce', [noise_signal, noise_signal.conj()]) / denom_noise
            ...
            speech_signals = [data * mask_s for mask_s in mask_speech]
            noise_signals = [data * mask_n for mask_n in mask_noise]
            # (B, F, 1, 1)
            denom_speech = [
                (speech_signal.real.pow(2) + speech_signal.imag.pow(2)).sum(
                    dim=(-1, -2), keepdims=True
                )
                for speech_signal in speech_signals
            ]
            denom_noise = [
                (noise_signal.real.pow(2) + noise_signal.imag.pow(2)).sum(
                    dim=(-1, -2), keepdims=True
                )
                for noise_signal in noise_signals
            ]

            # covariance of source speech
            psd_speeches = [
                # get_power_spectral_density_matrix(data, mask) for mask in mask_speech
                FC.einsum('...ct,...et->...ce', [speech_signal, speech_signal.conj()]) / denom_speech[spk]
                for spk, speech_signal in enumerate(speech_signals)
            ]
            if self.beamformer_type in ('mvdr', 'mvdr_souden'):
                # covariance of noise
                psd_noise = [
                    # get_power_spectral_density_matrix(data, maskn) for maskn in mask_noise
                    FC.einsum('...ct,...et->...ce', [noise_signal, noise_signal.conj()]) / denom_noise[spk]
                    for spk, noise_signal in enumerate(noise_signals)
                ]
            elif self.beamformer_type == 'mpdr':
                # covariance of observed speech
                psd_observed = FC.einsum('...ct,...et->...ce', [data, data.conj()])
    ...
    def forward(self, x: ComplexTensor,
        ...
        if h.dim() == 4:
            ...
            if masks is not None:
                ...
                wpe_masks = [_revert_compressed_mask(m) for m in mask[:2]]
                beamforming_masks = [_revert_compressed_mask(m) for m in mask[2:]]
