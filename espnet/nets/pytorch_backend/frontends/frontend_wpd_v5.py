...
def get_WPD_filter_conj(Rf: ComplexTensor,
                        ...
                        eps: float = 1e-15,
                        glb_cov: ComplexTensor = None) -> ComplexTensor:
    ...
    Args:
        ...
        eps (float):
        glb_cov (ComplexTensor): (B, F, (btaps+1) * C, (btaps+1) * C)
            is the global statistics of the power normalized spatio-temporal
            covariance matrix on training set (reverberant + dereverberated).
...
#                                             torch.rand_like(Rf.real)) * 1e-4
            reg_coeff_tensor = glb_cov * 1e-4 if glb_cov is not None else \
...
#                                             torch.rand_like(Rf.real)) * 1e-1
            reg_coeff_tensor = glb_cov * 1e-1 if glb_cov is not None else \
...
#            self.beamformer = None3
        self.glb_cov = torch.load('/mnt/lustre/sjtu/users/wyz97/work_dir/wyz97/espnet-v0.5.2/multi-channel/wsj-mix-spatialized/data/tr_spatialized_reverb_multich_singlespkr/global_cov.pt')
    ...
    def forward(self, x: ComplexTensor,
        ...
        if self.glb_cov is not None:
            global_cov = self.glb_cov.to(covariance_matrix1.device)
            global_cov.requires_grad = False
        else:
            global_cov = None
        # (B, F, (btaps+1) * C)
        WPD_filter_conj1 = get_WPD_filter_conj(covariance_matrix1, psd_speech1, u1, glb_cov=global_cov)
        WPD_filter_conj2 = get_WPD_filter_conj(covariance_matrix2, psd_speech2, u2, glb_cov=global_cov)
