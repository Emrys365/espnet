...
from espnet.nets.pytorch_backend.frontends.complex import Solve
from espnet.nets.pytorch_backend.frontends.complex import hermite
from espnet.nets.pytorch_backend.frontends.complex import Tensor_to_ComplexTensor
from espnet.nets.pytorch_backend.frontends.complex import ComplexTensor_to_Tensor
...
def get_filter_matrix_conj(
    correlation_matrix: ComplexTensor,
    ...
    if diagonal_loading:
        ...
        eye = eye.reshape(1, correlation_matrix.size(-1), correlation_matrix.size(-1)).repeat(F, 1, 1)
        with torch.no_grad():
            epsilon = FC.trace(correlation_matrix).real.abs()[..., None, None] * eps
            # in case that correlation_matrix is all-zero
            epsilon = epsilon + 1e-8
        correlation_matrix2 = correlation_matrix + epsilon * eye
    else:
        eye = torch.eye(correlation_matrix.size(-1),
                        dtype=correlation_matrix.dtype,
                        device=correlation_matrix.device)
        ...
        eye = eye.view(*shape)
        correlation_matrix2 = correlation_matrix + 1e-10 * eye

    # (F, C1, taps * C2)
    if use_torch_solver:
        stacked_filter_conj = FC.solve(
            correlation_vector.transpose(-1, -2),
            correlation_matrix2
        )[0].transpose(-1, -2)
    else:
        stacked_filter_conj = FC.matmul(
            correlation_vector,
            correlation_matrix2.inverse().transpose(-1, -2)
        )
...
def wpe_one_iteration(
    ...
    if inverse_power:
        if mask_flooring:
            inverse_power = 1 / torch.clamp(power, min=1e-3)
        else:
