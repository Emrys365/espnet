...
class EncoderMix(Encoder, torch.nn.Module):
    ...
    def forward(self, xs, masks):
        ...
        """
#        print(f'[input] xs:{xs.shape}, masks:{masks.shape}', flush=True)
...
#        print(f'[embed] xs:{xs.shape}, masks:{masks.shape}', flush=True)
        for ns in range(self.num_spkrs):
            xs_sd[ns], masks_sd[ns] = self.encoders_sd[ns](xs, masks)
#            print(f'[encoders_sd] xs_sd[{ns}]:{xs_sd[ns].shape}, masks_sd[{ns}]:{masks_sd[ns].shape}', flush=True)
            xs_sd[ns], masks_sd[ns] = self.encoders(xs_sd[ns], masks_sd[ns])  # Enc_rec
#            print(f'[encoders] xs_sd[{ns}]:{xs_sd[ns].shape}, masks_sd[{ns}]:{masks_sd[ns].shape}', flush=True)
