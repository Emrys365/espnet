...
#    inv_correlation_matrix = correlation_matrix2.inverse2()
...
#                                    inv_correlation_matrix.transpose(-1, -2))
    ...
    ##stacked_filter, _ = torch.solve(
    ##    ComplexTensor_to_Tensor(correlation_vector),
    ##    ComplexTensor_to_Tensor(correlation_matrix)
    ##)
    ##stacked_filter_conj = hermite(Tensor_to_ComplexTensor(stacked_filter))

#    stacked_filter_conj = FC.matmul(
#        correlation_vector,
#        Tensor_to_ComplexTensor(
#            (ComplexTensor_to_Tensor(correlation_matrix)).inverse()
#        ).transpose(-1, -2)
#    )
...
def wpe_one_iteration(Y: ComplexTensor,
    ...
    if inverse_power:
        ...
        # add a regularization term
        #with torch.no_grad():
        #    epsilon = power.max(dim=-1, keepdim=True)[0] * eps
        #inverse_power = 1 / (power + epsilon)
    else:
        inverse_power = power
